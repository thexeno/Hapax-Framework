...
static spi_hal_conf_t *spi_hal_conf_buff;
...
static void spi_hal_spi2_isr()
{
    if (spi_hal_conf_buff[SPI_HAL_ENUM_SPI2].periph->CR2 & SPI_CR2_TXEIE)
    {
        spi_hal_ISR_txe_cb[SPI_HAL_ENUM_SPI2](spi_hal_conf_buff[SPI_HAL_ENUM_SPI2].spi);
    }
    else if (spi_hal_conf_buff[SPI_HAL_ENUM_SPI2].periph->CR2 & SPI_CR2_RXNEIE)
    {
        spi_hal_ISR_rxc_cb[SPI_HAL_ENUM_SPI2](spi_hal_conf_buff[SPI_HAL_ENUM_SPI2].spi);
    }
    else if (spi_hal_conf_buff[SPI_HAL_ENUM_SPI2].periph->CR2 & SPI_CR2_ERRIE)
    {
        spi_hal_ISR_err_cb[SPI_HAL_ENUM_SPI2](spi_hal_conf_buff[SPI_HAL_ENUM_SPI2].spi);
    }
    else
    {
        /* code */
...
static void spi_hal_spi1_isr()
{
    if (spi_hal_conf_buff[SPI_HAL_ENUM_SPI1].periph->CR2 & SPI_CR2_TXEIE)
    {
        spi_hal_ISR_txe_cb[SPI_HAL_ENUM_SPI1](spi_hal_conf_buff[SPI_HAL_ENUM_SPI1].spi);
    }
    else if (spi_hal_conf_buff[SPI_HAL_ENUM_SPI1].periph->CR2 & SPI_CR2_RXNEIE)
    {
        spi_hal_ISR_rxc_cb[SPI_HAL_ENUM_SPI1](spi_hal_conf_buff[SPI_HAL_ENUM_SPI1].spi);
    }
    else if (spi_hal_conf_buff[SPI_HAL_ENUM_SPI1].periph->CR2 & SPI_CR2_ERRIE)
    {
        spi_hal_ISR_err_cb[SPI_HAL_ENUM_SPI1](spi_hal_conf_buff[SPI_HAL_ENUM_SPI1].spi);
    }
    else
    {
        /* code */
...
spi_hal_err_t Spi_hal_ISR_callback_set(conf_spi_e spi, spi_hal_isr_t isr_type, void (*fp_t)(conf_spi_e))
{
    ...
    for (i = 0; i < SPI_TOTAL_INSTANCE; i++)
    {
        if (spi_hal_conf_buff[i].spi == spi)
        {
            ...
            {
                spi_hal_ISR_err_cb[spi_hal_conf_buff[i].spi] = fp_t;
            }
            else if (isr_type == SPI_HAL_ISR_T_TXE)
            {
                spi_hal_ISR_txe_cb[spi_hal_conf_buff[i].spi] = fp_t;
            }
            else if (isr_type == SPI_HAL_ISR_T_RX)
            {
                spi_hal_ISR_rxc_cb[spi_hal_conf_buff[i].spi] = fp_t;
            }
            else
            {
    ...
    spi_hal_err_t Spi_hal_init(spi_hal_conf_t * handle)
    {
        ...
        {
            spi_hal_ISR_err_cb[spi_hal_conf_buff[i].spi] = spi_hal_cb_default;
            spi_hal_ISR_txe_cb[spi_hal_conf_buff[i].spi] = spi_hal_cb_default;
            spi_hal_ISR_rxc_cb[spi_hal_conf_buff[i].spi] = spi_hal_cb_default;
            if (handle[i].spi != CONF_SPI_ENUM_UNUSED)
            {
                ...
                if (handle[i].periph == SPI_HAL_1)
                {
                    IntHal_vector_register(spi_hal_spi1_isr, SPI1_IRQHandler_num);
                }
                else if (handle[i].periph == SPI_HAL_2)
                {
                    IntHal_vector_register(spi_hal_spi2_isr, SPI2_IRQHandler_num);
                }
                else
                {
                    /* nothing here */
    ...
    spi_hal_err_t Spi_hal_transfer(spi_hal_handle_t handle, uint32_t txd, uint32_t * rxd)
    {
        ...
        else if (spi_hal_conf_buff[handle].status == SPI_HAL_ERR_LOCK)
        {
            return SPI_HAL_ERR_LOCK;
        }
        else
